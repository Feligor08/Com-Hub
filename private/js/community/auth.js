function AuthPage() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    alert(`${view === 'login' ? 'Login' : 'Registrierung'} für: ${email}`);
  };
          ...
          <div className="nexus-card-footer">
            {view === 'login' ? (
              <>
                ...
                <span className="nexus-footer-text">Need an account? <strong onClick={() => setView('register')}>Sign up</strong></span>
              </>
            ) : (
              <>
                <span></span>
                <span className="nexus-footer-text">Already have an account? <strong onClick={() => setView('login')}>Sign in</strong></span>
              </>
            )}
          </div>
